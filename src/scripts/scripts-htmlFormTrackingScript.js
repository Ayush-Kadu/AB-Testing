...
const setupTriggerBasedCampaignsGlobal = async () => {
    ...
    try {
        const [smsCampaignsRes, emailCampaignsRes, whatsappCampaignsRes] = await Promise.all([
            phoneFromCookie ? fetch(`http://localhost:5008/api/campaign/get-active-sms-campaigns?userId=${userId}`).catch(() => null) : null,
            emailFromCookie ? fetch(`http://localhost:5008/api/campaign/get-active-email-campaigns?userId=${userId}`).catch(() => null) : null,
            phoneFromCookie ? fetch(`http://localhost:5008/api/campaign/get-active-whatsapp-campaigns?userId=${userId}`).catch(() => null) : null
        ]);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const triggerSMSCampaigns = async (phoneNumber, userId, isFormSubmission = false) => {
        try {
            ...
            const campaignsResponse = await fetch(`http://localhost:5008/api/campaign/get-active-sms-campaigns?userId=${userId}`, {
                method: 'GET',
            ...
            for (const campaign of activeSMSCampaigns) {
                if (campaign.category === 'Send Action' && campaign.subCategory === 'SMS') {
                    ...
                    const sendSMS = async () => {
                        try {
                            const smsResponse = await fetch('http://localhost:5008/api/script/send-sms', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    contact: phoneNumber,
    ...
    const triggerEmailCampaigns = async (emailAddress, userId, isFormSubmission = false) => {
        try {
            ...
            const campaignsResponse = await fetch(`http://localhost:5008/api/campaign/get-active-email-campaigns?userId=${userId}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            });
            ...
            for (const campaign of activeEmailCampaigns) {
                if (campaign.category === 'Send Action' && campaign.subCategory === 'Email') {
                    ...
                    const sendEmail = async () => {
                        try {
                            const emailResponse = await fetch('http://localhost:5008/api/script/send-email-campaign', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    email: emailAddress,
    ...
    const triggerWhatsAppCampaigns = async (phoneNumber, userId, isFormSubmission = false) => {
        try {
            ...
            const campaignsResponse = await fetch(`http://localhost:5008/api/campaign/get-active-whatsapp-campaigns?userId=${userId}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            });
            ...
            for (const campaign of activeWhatsAppCampaigns) {
                if (campaign.category === 'Send Action' && campaign.subCategory === 'WhatsApp') {
                    ...
                    const sendWhatsApp = async () => {
                        try {
                            const whatsappResponse = await fetch('http://localhost:5008/api/script/send-whatsapp', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
